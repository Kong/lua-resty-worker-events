...
upstream node.
2. broadcast an event to the local worker only
...
source. All following parameters are the named events generated by the event 
source.
...
local _M = {}  -- define my module table
_M.events = ev.event_list("my_module_event_source", "started", "event2")

-- Post my own 'started' event
ev.post(_M.events._source, _M.events.started)
